...
function findChecksum(spreadsheet) {
  return spreadsheet.map(row => {
    // const smallest = row.split('\t').map(i => parseInt(i)).reduce((acc, cur) => acc < cur ? acc : cur);
    // const largest = row.split('\t').map(i => parseInt(i)).reduce((acc, cur) => acc > cur ? acc : cur);
    // const difference = largest - smallest;

    return row.split('\t').map(i => parseInt(i)).reduce((acc, cur, index, array) => {
      const val = array.filter(i => i !== cur).map(i => i / cur).filter(i => Number.isInteger(i));
      const divisor = val.length ? val[0] : acc;
